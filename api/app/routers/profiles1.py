...
async def update_my_profile(
    ...
    # --- AUTO-VERIFY PROFILE ---
    # When user manually reviews and saves their profile (after AI parsing),
    # this IS the human verification step. Mark profile as verified.
    if not profile.is_verified:
        profile.is_verified = True
        profile.verification_score = 1.0
        
        # Copy current profile state as validated_json
        profile.validated_json = {
            "personal_info": {
                "name": f"{db.get(User, current_user.user_id).full_name}" if db.get(User, current_user.user_id) else "",
                "email": db.get(User, current_user.user_id).email if db.get(User, current_user.user_id) else "",
            },
            "headline": profile.headline,
            "summary": profile.summary,
            "location_city": profile.location_city,
            ...
            "is_open_to_work": profile.is_open_to_work,
            "skills": [],
            "experience": profile.parsed_json_draft.get("experience", []) if isinstance(profile.parsed_json_draft, dict) else [],
            "education": profile.parsed_json_draft.get("education", []) if isinstance(profile.parsed_json_draft, dict) else [],
            "certifications": profile.parsed_json_draft.get("certifications", []) if isinstance(profile.parsed_json_draft, dict) else [],
        }
        
        # Populate skills in validated_json from profile_skills table
        profile_skills = db.execute(
            select(ProfileSkill, Skill)
            .join(Skill)
            .where(ProfileSkill.profile_id == profile.id)
        ...
        profile.validated_json["skills"] = [
            {
                "name": ps.Skill.name,
                "proficiency": ps.ProfileSkill.proficiency_level,
                "years": ps.ProfileSkill.years_experience
            }
            for ps in profile_skills
        ]

        # Try to generate embedding for verified profile
        try:
            ...
            # Embedding generation is optional, don't block save
...
@router.post("/me/quick-verify")
async def quick_verify_profile(
    current_user: TokenData = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """
    Quick verify - marks profile as verified using current profile data.
    No request body needed. Called when user clicks 'Verify Profile' button
    on the dashboard after reviewing their AI-parsed profile.
    """
    profile = db.execute(
        select(Profile).where(Profile.user_id == current_user.user_id)
    ).scalar_one_or_none()
    ...
    if profile.is_verified:
        return {"message": "Profile is already verified", "is_verified": True}

    # Build validated_json from current profile state
    user = db.get(User, current_user.user_id)
    ...
    # Add skills from profile_skills table
    profile_skills = db.execute(
        select(ProfileSkill, Skill)
        .join(Skill)
        .where(ProfileSkill.profile_id == profile.id)
